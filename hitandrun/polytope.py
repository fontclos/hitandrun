...
class Polytope(object):
    ...
    def __init__(self, A=None, b=None):
        ...
        self.auxiliar_points = np.zeros(shape=A.shape, dtype=np.float64)
    ...
    def _find_auxiliar_points_in_planes(self):
        ...
        auxiliar_points = np.zeros((self.nplanes, self.dim), dtype=np.float64)
    ...
    def _find_auxiliar_point(self, Ai, bi):
        ...
        p = np.zeros(self.dim, dtype=np.float64)
        j = np.argmax(Ai != 0)
        p[j] = bi / Ai[j]
        return p
